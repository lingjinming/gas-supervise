<template>
  <uni-swipe-action>
    <uni-swipe-action-item>
      <view class="data-box" @click="navigateToDetail(data.uid, data.planCode, $event)">
        <view class="tit">
        ...
        <view class="con">
          ...
          <view class="bottom">
            <view>
              <uni-icons type="person" size="18" />
              由<text class="label"> {{ data.planCreator }} </text>创建
            </view>

            <view class="state" :class="data.handleState == 'UNCOMPLETED'
                ? 'wait-handle-red'
                : 'handled'
              ">
              {{ data._handleState }}
